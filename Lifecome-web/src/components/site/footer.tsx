  ...
  return (
    <footer className="mt-auto border-t border-line bg-surface">
      <Container className="py-14">
              ...
              <div key={group.label}>
                ...
                <ul className="mt-3 space-y-2">
                  {group.children!.map((link) => (
                    <li key={link.href}>
                      <Link href={link.href} className="text-sm text-ink-muted hover:text-blue hover:underline">
                        {link.label}
                      </Link>
                    </li>
                  ))}
        ...
        <div className="mt-12 rounded-card border border-gold/50 bg-white p-4 text-sm text-ink">
          ...
          <Link href="/emergency" className="font-semibold text-blue underline underline-offset-2">
        ...
        <div className="mt-8 flex flex-col gap-4 border-t border-line pt-6 text-sm text-ink-muted sm:flex-row sm:items-center sm:justify-between">
          ...
          <ul className="flex flex-wrap gap-x-6 gap-y-2">
            {footerLegalLinks.map((link) => (
              <li key={link.href}>
                <Link href={link.href} className="hover:text-blue hover:underline">
                  {link.label}
                </Link>
              </li>
            ))}
